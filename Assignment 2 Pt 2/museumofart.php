...
//<!-- Claude Monet -->
    ...
    $conn = new mysqli($servername, $username, $password, $dbname);
    echo "<div class='content container-fluid' id='Mofart'>";
        $sql = "SELECT mname FROM Museum WHERE id='14'";
        $result = $conn->query($sql);
        if (mysqli_num_rows($result) > 0)
        {
        // output data of each row
            while($row = mysqli_fetch_assoc($result))
            {
                echo "<h1 class='font'>".$row['mname']."</h1>";
            }
        }
        else
        {
        ...
        echo "<div class='col-sm-3 col-md-6 col-lg-4'>";
            echo "<p>";
                $sql = "SELECT doe, location, address FROM Museum WHERE id='14'";
                $result = $conn->query($sql);
                if (mysqli_num_rows($result) > 0)
                {
                // output data of each row
                    while($row = mysqli_fetch_assoc($result))
                    {
                        echo "Location: ".$row['location']."<br><br> Address: ".$row['address'] . "<br><br> Date of establishing: ".$row['doe'] ." <br><br>";
                    }
                }
                else
                {
                ...
                $sql = "SELECT fname, lname FROM Builder WHERE id='14'";
                $result = $conn->query($sql);
                if (mysqli_num_rows($result) > 0)
                {
                // output data of each row
                ...
                $sql = "SELECT piece FROM FamousPieces WHERE id='14'";
                $result = $conn->query($sql);
                if (mysqli_num_rows($result) > 0)
                {
                // output data of each row
