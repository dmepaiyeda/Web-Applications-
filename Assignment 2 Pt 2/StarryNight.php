...
<div class="content container-fluid  font" id="starry">
    ...
    <?php   $servername = "localhost";
            ...
            $sql = "SELECT firstname, lastname, piece FROM Artwork WHERE idArtwork='2'";
            $result = $conn->query($sql);
            if (mysqli_num_rows($result) > 0)
            {
            // output data of each row
        ...
        echo "<div class='col-sm-9 col-md-6 col-lg-6'>";
            ...
            // output data of each row
                while($row = mysqli_fetch_assoc($result))
                {
                    echo "<img id='snight' src='". $row["url"]."'>  <br><br>";
                }
            }
            else
            {
        ...
        echo "<div class='col-sm-3 col-md-6 col-lg-4'>";
            echo"<p>";
                        $sql = "SELECT year, type, dimension, price, location FROM Artwork WHERE idArtwork='2'";
                        $result = $conn->query($sql);
                        if (mysqli_num_rows($result) > 0) {
                        // output data of each row
                            while($row = mysqli_fetch_assoc($result)) {
                        ...
                        $sql = "SELECT genre FROM Genre WHERE id='2'";
                        $result = $conn->query($sql);
                        if (mysqli_num_rows($result) > 0) {
                        // output data of each row
                        echo "Genres: ";
