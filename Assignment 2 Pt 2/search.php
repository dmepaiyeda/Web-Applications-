  ...
  <?php
       ...
       $search = $_POST['search'];
             ...
             if( strlen( $search ) <= 1 )
                    ...
                    mysql_connect( "localhost","smaccall","pygleif0") ;
                    mysql_select_db("smaccall");
                    ...
                    $x = 0;
                     foreach( $search_exploded as $search_each ) {
                                        ...
                                        $construct = "SELECT * FROM Artwork WHERE $construct ";
                                        $run = mysql_query( $construct );
                                        $foundnum = mysql_num_rows($run);
                                       if ($foundnum == 0)
                                              echo "Sorry, there are no matching result for <b> $search </b>. </br> Please check your spelling";
                                       else {
                          echo "$foundnum results found !<p>";

                          while( $runrows = mysql_fetch_assoc( $run ) ) {
                                 $id = $runrows['idArtwork'];
                                 $piece = $runrows['piece'];
                                 $type= $runrows['type'];
                                 $dimension=$runrows['dimensions'];
                                 $year = $runrows['year'];
                                 $price = $runrows['price'];
                                 $firstname=$runrows['firstname'];
                                 $lastname = $runrows['lastname'];
                                 $location = $runrows['location'];
                                 $cslink = $runrows['cslink'];


                                 echo "<a href='$piece'> <b> $type </b> </a> <br> $price <br> <a href='$url'> $url </a> <p>";
