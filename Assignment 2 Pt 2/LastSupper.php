...
<div class="content container-fluid font" id="supper">
    ...
    <?php   $servername = "localhost";
            ...
            $sql = "SELECT firstname, lastname, piece FROM Artwork WHERE idArtwork='4'";
            $result = $conn->query($sql);
            if (mysqli_num_rows($result) > 0)
            {
            // output data of each row
        ...
        echo "<div class='col-sm-3 col-md-6 col-lg-4'>";
            echo"<p>";
                        $sql = "SELECT year, type, dimension, price, location FROM Artwork WHERE idArtwork='4'";
                        $result = $conn->query($sql);
                        if (mysqli_num_rows($result) > 0) {
                        // output data of each row
                            while($row = mysqli_fetch_assoc($result)) {
                        ...
                        $sql = "SELECT genre FROM Genre WHERE id='4'";
                        $result = $conn->query($sql);
                        if (mysqli_num_rows($result) > 0) {
                        // output data of each row
                        echo "Genres: ";
